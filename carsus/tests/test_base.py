...
def test_init_db(session):
    assert session.query(Atom).count() == 118
...
def test_atomic_database_test_atoms(atomic_number, expected, session):
    atom = session.query(Atom).filter_by(atomic_number=atomic_number).one()
